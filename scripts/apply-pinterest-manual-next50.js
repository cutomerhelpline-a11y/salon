...
const BACKUP = path.resolve('./entities/products-data.json.manual-pinterest-next50.bak');
...
// Next 50 pins provided by the user — mapped to products p051..p100 (index 50..99)
const pins = [
    ...
    // add five more from the tail of the user's list to reach 50
    'https://pin.it/2kRfzQQUL', 'https://pin.it/4iHxhM5GL', 'https://pin.it/4qHB0kLur', 'https://pin.it/4tVSNT7jt', 'https://pin.it/4yr8t7QFl'
];
...
async function main() {
    ...
    const START = 50; // map to products array index 50..99 (p051..p100)
    for (let i = 0; i < Math.min(pins.length, 50); i++) {
        const prodIndex = START + i;
        const p = products[prodIndex];
        const id = p?.id || `p${prodIndex + 1}`;
        const pin = pins[i];
    ...
    fs.writeFileSync(path.resolve(ART, 'remap-pinterest-manual-next50.csv'), csv, 'utf8');
    console.log('Wrote', path.resolve(ART, 'remap-pinterest-manual-next50.csv'));
}
